...
describe('Performance: Sequencer Throughput', () => {
    ...
    it('should measure single-threaded submitMany throughput', async () => {
        ...
        console.log(`Sequencer (Single): ${end - start}ms (${tps} tx/sec)`);

        await sequencer.stop();
    }, 60000);

    it('should measure parallel submitMany throughput', async () => {
        ...
        expect(results.every(r => r.accepted)).toBe(true);
        const tps = (count / ((end - start) / 1000)).toFixed(2);
        console.log(`Sequencer (Parallel): ${end - start}ms (${tps} tx/sec)`);
